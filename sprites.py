...
class Demon(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        self.groups = game.all_sprites, game.demon
        pg.sprite.Sprite.__init__(self, self.groups)
        self.game = game
        self.image = game.demon_img.copy()
        self.rect = self.image.get_rect()
        self.rect.center = (x, y)
        self.hit_rect = DEMON_HIT_RECT.copy()
        self.hit_rect.center = self.rect.center
        self.pos = vec(x, y)
        self.vel = vec(0, 0)
        self.acc = vec(0, 0)
        ...
        self.speed = choice(DEMON_SPEEDS)
        self.target = game.player

    def avoid_demon(self):
        for demon in self.game.demon:
            if demon != self:
                dist = self.pos - demon.pos
                if 0 < dist.length() < AVOID1_RADIUS:
    ...
    def update(self):
        ...
        if target_dist.length_squared() < DETECT_RADIUS**2:
            ...
            self.image = pg.transform.rotate(self.game.demon_img, self.rot)
            self.rect.center = self.pos
    ...
    def draw_health1(self):
        if self.health > 60:
            ...
            col = RED
        width = int(self.rect.width * self.health / DEMON_HEALTH)
        self.health_bar = pg.Rect(0, 0, width, 7)
        if self.health < DEMON_HEALTH:
